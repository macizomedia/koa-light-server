...
exports.login = async (ctx) => {
	try {
		const data = matchedData(ctx.request.body);
		console.log(data.email);
		ctx.state.user = await findUser(data.email);
		await userIsBlocked(ctx.state.user);
		await checkLoginAttemptsAndBlockExpires(ctx.state.user);
		const isPasswordMatch = await auth.checkPassword(data.password, ctx.state.user);
		if (!isPasswordMatch) {
			utils.handleError(ctx.res, await passwordsDoNotMatch(ctx.state.user));
		} else {
			// all ok, register access and return token
			ctx.state.user.loginAttempts = 0;
			await saveLoginAttemptsToDB(ctx.state.user);
			ctx.status(200).json(await saveUserAccessAndReturnToken(ctx.res, ctx.state.user));
		}
	} catch (error) {
